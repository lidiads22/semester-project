...
public class PizzaControls extends JPanel {
	...
	private String[] pizzaStyle = { "Hand Tossed", "Thin", "Deep Dish" };
...
cboPizzaStyle.setFont(new Font("Helvetica", Font.BOLD,
16));
...
pnlName = new JPanel();
pnlName.setBorder(BorderFactory.createLineBorder(Color.WHITE,
3));
pnlName.add(lblName);
pnlName.add(txtName);
...
pnlPizza.setLayout(new BoxLayout(pnlPizza,
BoxLayout.X_AXIS));
...
pnlNameAndPizza.setBorder(BorderFactory.createLineBorder(Color.BLACK, 5));
		...
		pnlNameAndPizza.add(Box.createRigidArea(new
		Dimension(5,0)));
		...
		pnlToppings1.setBorder(BorderFactory.createCompoundBorder(border1
		, border2));
		...
		pnlAllToppings.add(Box.createRigidArea(new
		Dimension(0,5)));
		...
		pnlColOne.setLayout(new BoxLayout(pnlColOne,
		BoxLayout.Y_AXIS));
		...
		pnlComplete.setBorder(BorderFactory.createLineBorder(Color.BLACK,
		5));
		...
		pnlMoney = new JPanel();
		pnlMoney.setLayout(new BoxLayout(pnlMoney,
		BoxLayout.Y_AXIS));
		pnlMoney.setBorder(BorderFactory.createLineBorder(Color.BLACK,
		5));
		pnlMoney.add(lblCost);
		pnlMoney.add(Box.createRigidArea(new Dimension(0,10)));
		pnlPaid = new JPanel();
		pnlPaid.setLayout(new BoxLayout(pnlPaid,
		BoxLayout.X_AXIS));
		pnlPaid.add(lblPaid);
		...
		pnlColTwo.setLayout(new BoxLayout(pnlColTwo,
		BoxLayout.Y_AXIS));
		...
		pnlOrder = new JPanel();
		pnlOrder.setLayout(new BoxLayout(pnlOrder,
		BoxLayout.Y_AXIS));
	...
	private class PizzaSizeListener implements ActionListener {
		public void actionPerformed(ActionEvent event)
		{
			...
			if (gotName)
			{
				...
				else if (selection == 1) // medium pizza
				{
	...
	private class Toppings2Listener implements ItemListener {
		public void itemStateChanged(ItemEvent event) {
			...
			// see which toppings are selected 
	...
	private class TransactionListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			if (source == rbtCash || source == rbtCard)
			{
			   ...
			   // additional charge 50 cents added
	...
	private class ExitListener implements ActionListener {
		...
		{
		 JOptionPane.showMessageDialog(null,
		  "This program was written by Lidia De Santiago" +
		  "\nEnd of program.",
		  "Good bye!",
